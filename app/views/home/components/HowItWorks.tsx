        ...
        {steps.map((step, index) => (
          <article className={`how-step how-step-${index + 1}`} key={step.number}>
            ...
            <div className="how-copy">
              <div className="how-title-row"><span className="how-number">{step.number}</span><h3>{step.title}</h3></div>
              <div className="how-body">{step.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>
