  ...
  return (
    <section className="gift-card min-h-[449px] rounded-[20px] bg-[#97ff77] px-6 pt-[79px] pb-[68px] text-center text-[#2E2E38] max-[700px]:min-h-0 max-[700px]:rounded-[18px] max-[700px]:px-[22px] max-[700px]:pt-[31px] max-[700px]:pb-[34px]" aria-labelledby="gift-card-heading">
      <div className="gift-card-content mx-auto w-full max-w-[780px]">
        ...
        <p className="mt-[22px] text-[16px] leading-[1.45] max-[700px]:mt-5 max-[700px]:text-[15px] max-[700px]:leading-[1.48] [&>br]:max-[700px]:hidden">
        ...
        <form className="gift-card-form mx-auto mt-6 flex max-w-[480px] flex-col items-center" onSubmit={(event) => { event.preventDefault(); if (!canSubmit) { setAmountError("Please enter an amount between $1 and $10,000 USD."); return; } setAmountError(""); setPurchaseAmount(amountValue); setIsOpen(true); }}>
      ...
      {isOpen && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#263443]/60 px-4 py-8" role="dialog" aria-modal="true" aria-labelledby="voucher-modal-title">
        <div className="voucher-modal relative max-h-full w-full max-w-[520px] overflow-y-auto rounded-[24px] bg-white p-8 text-left text-[#263443] shadow-2xl max-[600px]:p-5">
          ...
          {voucherCode ? <div className="py-5 text-center"><h2 id="voucher-modal-title" className="voucher-modal-heading">Your voucher is ready</h2><p className="mt-4">Your digital voucher has been emailed to <em>{voucherEmail}</em>. You can also copy your unique code below:</p><div className="mt-5 flex items-center gap-2 rounded-xl bg-[#97ff77] p-2"><p className="flex-1 px-2 py-2 text-[22px] font-semibold tracking-wider">{voucherCode}</p><button type="button" className="rounded-full bg-white px-4 py-2 text-[14px] font-semibold text-[#263443] shadow-sm transition hover:bg-[#f2fff0]" onClick={async () => { await navigator.clipboard.writeText(voucherCode); setCopied(true); setTimeout(() => setCopied(false), 1800); }}>{copied ? "COPIED" : "COPY"}</button></div><p className="mt-4 text-[13px] leading-[1.5] text-[#515151]">Didn&apos;t receive the email? Check your spam folder or contact <a className="font-semibold text-[#008d60] underline underline-offset-2" href="mailto:hello@kincollage.com">hello@kincollage.com</a>.</p></div> : paymentState.status === "ready" ? <><h2 id="voucher-modal-title" className="voucher-modal-heading">{paymentMethodSelected ? "Complete your purchase" : "Select payment method"}</h2><VoucherPaymentForm clientSecret={paymentState.clientSecret!} amountCents={paymentState.amountCents!} onComplete={(code, email) => { setVoucherCode(code); setVoucherEmail(email || paymentState.email || ""); setAmount(""); }} onPaymentMethodChange={setPaymentMethodSelected} /></> : <><h2 id="voucher-modal-title" className="voucher-modal-heading">Where should we send it?</h2><p className="mt-2 text-[15px] text-[#515151]">Enter the recipient&apos;s email address below. They&apos;ll receive their digital voucher as soon as you complete payment via our secure Stripe checkout.</p><form action={paymentAction} className="mt-6"><input type="hidden" name="amount" value={purchaseAmount} /><label className="font-[var(--font-tenor-sans)] text-[14px]">EMAIL ADDRESS<input className="mt-2 h-11 w-full rounded-full border-2 border-[#aaaab5] px-4 font-[var(--font-montserrat)] outline-none focus:border-[#008d60]" name="email" type="email" placeholder="you@example.com" required /></label>{paymentState.status === "error" && !paymentState.message?.includes("amount") && <p className="commission-field-error mt-3" role="alert">{paymentState.message}</p>}<button className="button-primary mt-6 min-h-[51px] w-full rounded-full border-0 font-[var(--font-tenor-sans)] text-[15px] uppercase tracking-[.03em]" type="submit" disabled={isCreatingPayment}>{isCreatingPayment ? "PREPARING SECURE PAYMENT…" : "CONTINUE TO PAYMENT"}</button><p className="mt-3 text-center text-[13px] text-[#515151]">🔒 Safe &amp; secure checkout via Stripe</p></form></>}
        </div>
      </div>}
    </section>
  );
