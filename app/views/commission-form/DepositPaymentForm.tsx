...
function PayButton({ amountCents, currency, paymentLabel, onSuccess }: { amountCents: number; currency: string; paymentLabel: string; onSuccess: (paymentIntentId: string) => void }) {
  ...
  return (
    <form className="commission-payment-form mt-6" onSubmit={handlePay}>
      ...
      <button className="button-primary commission-order-submit mt-6" type="submit" disabled={!stripe || !elements || !isPaymentComplete || isPaying}>
        {isPaying ? "Processing…" : paidPaymentIntentId ? "Retry order confirmation" : `${paymentLabel} — ${money.format(amountCents / 100)}`}
      </button>
    </form>
  );
}

export default function DepositPaymentForm({ clientSecret, amountCents, totalCents, currency, paymentPlan, onSuccess }: {
  clientSecret: string;
  amountCents: number;
  totalCents: number;
  currency: string;
    ...
    <div className="commission-payment mt-8 rounded-[18px] border border-[#e6e6eb] bg-white p-6 max-[700px]:p-5">
      ...
      <p className="mt-2 text-[14px] text-[#515151]">
        {paymentPlan === "installments"
          ? <>2026 installment total {money.format(totalCents / 100)} — installment 1 of 3 due now: <strong>{money.format(amountCents / 100)}</strong>. The final balance is due before dispatch.</>
          : <>Total {money.format(totalCents / 100)} — full payment due now: <strong>{money.format(amountCents / 100)}</strong>.</>}
      </p>
      <Elements stripe={getStripe()} options={{
        ...
        appearance: {
          theme: "flat",
          variables: { colorPrimary: "#00b982", colorText: "#263443", colorDanger: "#ad3127", fontFamily: "'Tenor Sans', Arial, sans-serif", borderRadius: "10px", spacingUnit: "4px" },
          rules: { ".Input": { border: "2px solid #aaaab5", padding: "10px 14px" }, ".Input:focus": { border: "2px solid #00b982", boxShadow: "0 0 0 3px rgb(0 209 143 / 17%)" }, ".Label": { fontSize: "13px", textTransform: "uppercase", color: "#565661" } },
        },
      }}>
        <PayButton amountCents={amountCents} currency={currency} paymentLabel={paymentPlan === "installments" ? "Pay installment" : "Pay in full"} onSuccess={onSuccess} />
      </Elements>
    </div>
  );
}
