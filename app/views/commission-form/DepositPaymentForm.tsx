...
import { completeCommissionOrder, savePaymentCustomerDetails, updateCommissionPaymentOptions } from "../../actions/commissionDeposit";
...
function CheckoutSummary({ items, amountCents, totalCents, currency, paymentPlan, options }: { items: CheckoutItem[]; amountCents: number; totalCents: number; currency: string; paymentPlan: "full" | "installments"; options: PaymentOptions }) {
  ...
  const rushCents = Math.max(0, totalCents - baseTotalCents - options.shippingCents);
...
export default function DepositPaymentForm({ clientSecret, amountCents, totalCents, currency, paymentPlan, sizeLabel, items, onSuccess }: { clientSecret: string; amountCents: number; totalCents: number; currency: string; paymentPlan: "full" | "installments"; sizeLabel: string; items: CheckoutItem[]; onSuccess: (paymentIntentId: string) => void }) {
  ...
  const handleOptionsChange = (next: PaymentOptions) => {
    ...
    const amount = paymentPlan === "installments" ? baseAmount + Math.round(rush / 3) + next.shippingCents : baseAmount + rush + next.shippingCents;
    setOptions(next);
    setDisplayAmountCents(amount);
    setDisplayTotalCents(baseTotal + rush + next.shippingCents);
    return { amountCents: amount, totalCents: baseTotal + rush + next.shippingCents };
  };
  return (
    <div className="commission-checkout-shell">
      <CheckoutSummary items={items} amountCents={displayAmountCents} totalCents={displayTotalCents} currency={currency} paymentPlan={paymentPlan} options={options} />
      <section className="commission-checkout-form-panel">
        <h3>Shipping information</h3>
        <Elements stripe={getStripe()} options={{ clientSecret, fonts: [{ cssSrc: "https://fonts.googleapis.com/css2?family=Tenor+Sans&display=swap" }], appearance: { theme: "flat", variables: { colorPrimary: "#00b982", colorText: "#263443", colorDanger: "#ad3127", fontFamily: "'Tenor Sans', Arial, sans-serif", fontSizeBase: "14px", fontSizeSm: "13px", borderRadius: "8px", spacingUnit: "4px" }, rules: { ".Input": { border: "1px solid #d6d6dc", borderRadius: "7px", backgroundColor: "#fff", padding: "9px 12px", boxShadow: "0 1px 3px rgb(38 52 67 / 10%)", fontSize: "14px" }, ".Input:focus": { border: "2px solid #00b982", boxShadow: "0 0 0 3px rgb(0 209 143 / 17%)" }, ".Label": { fontSize: "13px", textTransform: "none", color: "#565661" } } } }}>
          <PayButton amountCents={displayAmountCents} currency={currency} paymentLabel={paymentPlan === "installments" ? "Pay installment" : "Pay in full"} paymentIntentId={clientSecret.split("_secret")[0]} sizeLabel={sizeLabel} onOptionsChange={handleOptionsChange} onSuccess={onSuccess} />
        </Elements>
      </section>
    </div>
  );
