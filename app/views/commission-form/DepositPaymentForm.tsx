  ...
  return (
    <form className="commission-payment-form" onSubmit={handlePay}>
      ...
      <fieldset className="commission-stripe-shipping-field"><legend>Shipping method</legend><div className="commission-shipping-options">
        ...
        {shippingRates.options.map((option) => <label key={option.id}><input type="radio" name="stripeShippingRegion" checked={shippingRegion === option.id} onChange={() => changeOptions({ priorityDate, shippingCents: option.amount, shippingRegion: option.id })} /><span>{option.label}{option.deliveryEstimate && <small>{option.deliveryEstimate}</small>}</span><strong>{formatPrice(option.amount)}</strong></label>)}
      </div></fieldset>
